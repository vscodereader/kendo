...
function ContactListPage() {
  ...
  const load = async (pageNumber: number, query: string, searchField: 'all' | 'title' | 'content') => {
    setLoading(true);
    try {
        ...
        <div className="contact-list-card">
          <div className="contact-search-card">
            ...
            <div className="contact-search-row">
              <select value={field} onChange={(e) => setField(e.target.value as 'all' | 'title' | 'content')}>
          ...
          <div className="contact-count-row">
            총 {loading ? 0 : data?.totalCount ?? 0}건 · {loading ? page : data?.currentPage ?? 1}/{loading ? 1 : data?.totalPages ?? 1} 페이지
          </div>
            ...
            <tbody>
              {loading ? (
                ...
                data.items.map((item, index) => {
                  const no = (data.totalCount ?? 0) - ((data.currentPage ?? 1) - 1) * 10 - index;
          ...
          <div className="contact-list-bottom-row">
            ...
            <div className="contact-pagination">
              <button className="page-btn" disabled={!data || page === 1} onClick={() => setPage(1)}>
                |&lt;
              </button>
              <button className="page-btn" disabled={!data || page === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>
                &lt;
              </button>
              ...
              <button className="page-btn" disabled={!data || page === (data?.totalPages ?? 1)} onClick={() => setPage(data?.totalPages ?? 1)}>
                &gt;|
              </button>
            </div>
