...
function NoticeDetailPage() {
  ...
  const [post, setPost] = useState<NoticePostDetail | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try {
        const result = await apiRequest<NoticePostDetail>(`/club/notice/posts/${postId}`);
        setPost(result);
      } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm('정말 이 공지글을 삭제하시겠습니까?');
    if (!confirmed) return;

    setDeleting(true);
    try {
      await apiRequest(`/club/notice/posts/${postId}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="page-shell themed-page-shell">
      <div className="notice-detail-page-wrap">
        <div className="notice-detail-topbar--outside">
          <button className="danger-outline-btn" onClick={() => navigate('/notice')}>
            목록으로
          </button>
        </div>
        ...
        {canManage ? (
          <div className="notice-detail-card-actions">
            <button className="primary-btn" onClick={() => navigate(`/notice/${post.id}/edit`)}>
              수정
            </button>
