...
function EventsWritePage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try {
        const post = await apiRequest<EventPostDetail>(`/club/events/posts/${postId}`);
        setTitle(post.title);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await apiRequest<EventPostDetail>(
        isEditMode ? `/club/events/posts/${postId}` : '/club/events/posts',
        {
      ...
      pushToast(isEditMode ? '대회 및 심사 일정 공지가 수정되었습니다.' : '대회 및 심사 일정 공지가 저장되었습니다.', 'success');
    ...
    <div className="page-shell">
      <div className="notice-write-card">
        <div className="notice-write-header">
          <h1>{isEditMode ? '대회 및 심사 일정 공지 수정' : '대회 및 심사 일정 공지 작성'}</h1>
        </div>
              ...
              {attachments.length === 0 ? (
                ...
                attachments.map((file, index) => (
                  <div key={`${file.name}-${file.size}-${file.lastModified}`} className="notice-file-item">
                    <div className="notice-file-item__name">📎 {file.name}</div>
                    <div className="notice-file-item__meta">{(file.size / 1024 / 1024).toFixed(2)} MB</div>
                    <button
                      type="button"
                      className="ghost-btn"
                      onClick={() =>
        ...
        <div className="notice-write-actions">
          ...
          <button className="ghost-btn" onClick={() => navigate('/events')} disabled={saving}>
            취소
          </button>
        </div>
      </div>
...
export default EventsWritePage;